      ...
      <div style={s.sidebar}>
        ...
        <div style={s.userInfo}>
          ...
          <div>
            ...
            <div style={s.userRole}>Tutor</div>
          </div>
        </div>
        <nav style={s.nav}>
          <button style={{...s.navBtn, ...s.navActive}}>📋 Pending Approvals</button>
        </nav>
        <button style={s.logoutBtn} onClick={logout}>Sign Out</button>
      </div>

      <div style={s.main}>
        <h1 style={s.pageTitle}>Pending Approvals ({leaves.length})</h1>
        {loading ? <p>Loading...</p> : leaves.length === 0
          ? <p style={{color:'#636e72'}}>No pending approvals. All caught up!</p>
          : leaves.map(leave => (
            <div key={leave._id} style={s.leaveCard}>
              <div style={s.leaveTop}>
                <div>
                  <span style={s.studentName}>{leave.student?.name}</span>
                  <span style={s.rollNum}> · {leave.student?.rollNumber} · {leave.student?.department}</span>
                </div>
              ...
              <div style={s.leaveDetails}>
                <span style={{textTransform:'capitalize', fontWeight:'500'}}>{leave.type}</span>
...
const s = {
  ...
  avatar:      { width:'40px', height:'40px', borderRadius:'50%', background:'#a29bfe', color:'#fff', display:'flex', alignItems:'center', justifyContent:'center', fontWeight:'700', fontSize:'1.1rem' },
  ...
  leaveDetails:{ display:'flex', gap:'1rem', color:'#636e72', fontSize:'0.85rem', margin:'0.5rem 0' },
  leaveReason: { color:'#636e72', fontSize:'0.9rem' },
